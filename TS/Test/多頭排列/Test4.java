...
public class Test4 {
 ...
 static Hashtable ht ;
  ...
  static public boolean checkValue(Line l, float cfv)
  {
        ...
        if(v2.getValue() > 0 && v1.getValue() < 0) return true;
...
//         TestKD   skd = new TestKD(check_snum[k],arg[0],1);
         MALine m1 =  skd.culMALine(1);
         MALine m8 =  skd.culMALine(8);
         MALine m13 =  skd.culMALine(13);
         MALine m21 =  skd.culMALine(21);
         MALine m33 =  skd.culMALine(33);

         Line sub_m1 = m1.sub(m1.length()-5,m1.length()-1);
         Line sub_m8 = m8.sub(m8.length()-5,m8.length()-1);
         Line sub_m8_x = m8.sub(m8.length()-15,m8.length()-5);
         Line sub_m13 = m13.sub(m13.length()-15,m13.length()-5);
         Line sub_m21 = m21.sub(m21.length()-15,m21.length()-5);
         Line sub_m33 = m33.sub(m33.length()-15,m33.length()-5);
         

         Line dif_0 = sub_m1.diff(sub_m8);
         Line dif_1 = sub_m8_x.diff(sub_m13);
         Line dif_2 = sub_m13.diff(sub_m21);
         Line dif_3 = sub_m21.diff(sub_m33);



        if(checkValue(dif_0,(float)0.1)&& checkValue1(dif_1,0) && checkValue1(dif_2,0) && checkValue1(dif_3,0))
         {
             System.out.println("snum "+ds000r.snum+" "+(new String(ds000r.sname.getBytes("ISO8859-1"),"MS950"))+ " "+
                        ds000r.capital+" OK! -->" + sub_m8.valueAt(0).getDateValue());
