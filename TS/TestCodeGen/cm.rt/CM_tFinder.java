...
static public void main(String[] arg)throws Exception{
      ...
      while(e.hasMoreElements())
      {
         ...
         if(check_snum != null && check_snum.get(ds000r.snum) == null)
         {
      	 ...
      	 total++;
         CM_sFactory   fac1 = new CM_sFactory(ds000r.snum,arg[0]);
         CM_oFactory   fac2 = new CM_oFactory(ds000r.snum,arg[0]);
         ...
         CM_Pattern   ptn1 = new CM_Pattern(markLine,basicGroup);
         try
         {
            PatternLine pl = (PatternLine)mainLine.pattern(ptn1);
            if(pl.length() > 0)
            {
               PatternValue ptv = (PatternValue)pl.valueAt(pl.length()-1);
               if(mainLine.length() - ptv.getEndIdx() < 2)
               { 
              	 got++;
