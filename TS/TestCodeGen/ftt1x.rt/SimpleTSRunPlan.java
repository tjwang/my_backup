...
public class  SimpleTSRunPlan implements TSRunPlan {
   ...
   public SimpleTSRunPlan(boolean iEy)
   {
      ...
      try
      {
      	if(up_idx_ma == null)
      	{
          up_idx_ma  =  (FeatureIdxHash)(new ObjectInputStream(new FileInputStream("fis_up_ma.idxh"))).readObject();
          System.out.println("up_idx_ma------------");
          up_idx_ma.dump();
        }
      	if(down_idx_ma == null)
      	{
          down_idx_ma  =  (FeatureIdxHash)(new ObjectInputStream(new FileInputStream("fis_down_ma.idxh"))).readObject();
          System.out.println("down_idx_ma------------");
          down_idx_ma.dump();
        }
      	if(up_idx_k == null)
      	{
          up_idx_k  =  (FeatureIdxHash)(new ObjectInputStream(new FileInputStream("fis_up_k.idxh"))).readObject();
          System.out.println("up_idx_k------------");
          up_idx_k.dump();
        }
      	if(down_idx_k == null)
      	{
          down_idx_k  =  (FeatureIdxHash)(new ObjectInputStream(new FileInputStream("fis_down_k.idxh"))).readObject();
          System.out.println("down_idx_k------------");
          down_idx_k.dump();
        }
      	if(up_idx_kd == null)
      	{
          up_idx_kd  =  (FeatureIdxHash)(new ObjectInputStream(new FileInputStream("fis_up_kd.idxh"))).readObject();
          System.out.println("up_idx_kd------------");
          up_idx_kd.dump();
        }
      	if(down_idx_kd == null)
      	{
          down_idx_kd  =  (FeatureIdxHash)(new ObjectInputStream(new FileInputStream("fis_down_kd.idxh"))).readObject();
          System.out.println("down_idx_kd------------");
          down_idx_kd.dump();
        }
      	if(up_idx_v == null)
      	{
          up_idx_v  =  (FeatureIdxHash)(new ObjectInputStream(new FileInputStream("fis_up_v.idxh"))).readObject();
          System.out.println("up_idx_v------------");
          up_idx_v.dump();
        }
      	if(down_idx_v == null)
      	{
          down_idx_v  =  (FeatureIdxHash)(new ObjectInputStream(new FileInputStream("fis_down_v.idxh"))).readObject();
        	...
        	// String _sta_fs_kd = culKDFs(kl_ex.sub(endIdx-25,endIdx-1));
        	 ...
        	 //String _sta_fs = culFs(min,mav3,mav8,mav13,max,v);
              ...
              if( up_p_ma > 0.8 && up_p_ma > down_p_ma && up_p_k > 0.5 && (up_p_v) > 0.5)
   	          ...
   	          } else if(down_p_ma > 0.5 && down_p_ma > up_p_ma && down_p_k > 0.5 && (down_p_v) > 0.5)
        	    {
