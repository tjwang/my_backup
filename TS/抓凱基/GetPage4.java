...
//             mnr = (PMname_Rec)e2.nextElement();
             for(int i = 0 ;i < 20;i++){
               try{
            	    while(!getData(dsr.snum,arg[1],arg[0])){
            	       i++;
            	       if(i > 20 ) break;
            	     }
       	          i = 501;
...
//       }
  ...
  static boolean getData(String snum,String mnum,String yearStr) throws Exception{
    ...
    int count = 0;
    URL u = new URL(u1,"zco0_"+snum+"_"+mnum+".djhtm");
    ...
    while((s=br.readLine())!=null){
         ...
         if(s.indexOf("class=\"t3") > 0)
         {
           ...
           { 
              ...
              sTotal = sb.toString();
              if(Integer.parseInt("1"+sTime.substring(0,2)) > 105)
              {
                 System.out.println(snum+"|"+"2009"+sTime.substring(0,2)+sTime.substring(3)+"|"+mnum+"|"+sBuy+"|"+sSell+"|"+sTotal+"|");
              }else{
