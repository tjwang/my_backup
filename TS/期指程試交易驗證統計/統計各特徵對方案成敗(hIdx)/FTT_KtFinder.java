...
static  String idxH_Path ="X:\\Working\\IdxH\\";
static  String logData_Path ="X:\\Working\\Data\\";
...
{   
	    try
	    {
         File dataFile = new File(logData_Path+ds+"\\fss_8888.log."+ds);
...
static void makeDir(String ds)
{
	    try
	    {
          File f = new File(idxH_Path+ds);
          f.mkdir();
      }catch(Exception xxe)
      {
        // xxe.printStackTrace();
    ...
    for(int xxi = SimpleTSRunPlan.UP; xxi < SimpleTSRunPlan.tMode_NUM; xxi++)
     for(int pmode = SimpleTSRunPlan.PATTEN_MA; pmode < SimpleTSRunPlan.tpMode_NUM; pmode++)
      for(int txRange = 0; txRange < 1 ; txRange++)
       for(int txGet = 20; txGet < 21 ; txGet++)
         ...
         S4_TransactionManager.one_instance = null;
            try
            {
                ...
                if(data_ary == null)
                {
                    ...
                    System.out.println(GMethod.d2s(sDate) +"-->"+data_ary);
                ...
                FileOutputStream fos = new FileOutputStream(idxH_Path+GMethod.d2s(sDate)+"\\Fis-"+tMode_str+"-"+tpMode_str+"."+GMethod.d2s(sDate)+".idxh");
