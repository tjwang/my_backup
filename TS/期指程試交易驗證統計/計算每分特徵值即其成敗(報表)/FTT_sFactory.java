...
//          TSRunPlan trp = new SimpleKRunPlan(true);
   ...
   void statisicStock(DomainValue dv, int xtime_i)
   {
       ...
       if(!ts_start && (!(_testTime - kl_ex.valueAt(kl_ex.length()-2).getTimeValue() < 100 &&  _testTime - kl_ex.valueAt(kl_ex.length()-2).getTimeValue() >= 0))) return ;
             ...
             {  
      	     ...
      	     if(code.equals(current_TX))
      	     {
      	       if(S4_TransactionManager.one_instance != null) 
      	       {
      	          ...
      	          {
//      	             System.out.println("xxxxxZx "+current_TX+" "+getLastValueTime(current_TX));
      	             S4_TransactionManager.one_instance.sRun();
      	          }
      	       }
      	     }
