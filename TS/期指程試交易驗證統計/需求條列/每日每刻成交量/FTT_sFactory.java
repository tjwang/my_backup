...
//      	 System.out.println(kl_ex);
    ...
    public Line getStaVLineByCode(String code, int t)
    {
    	   if(staVLinePool == null) staVLinePool = new Hashtable();
    	   Line l = (Line)staVLinePool.get(code+"_"+t);
    	   if(l == null)
    	   {
    	   	   try{
    	   	      stock.app.OneFieldBySQL of1 = new stock.app.OneFieldBySQL("select date, time, m_vol as f1  from sta_vol_n where sa_code ='"+
    	   	                                       code+"' and time='"+t+"' order by date;");
     	          l=of1.culF1Line();
    	          staVLinePool.put(code+"_"+t,l);
   	         }catch(Exception xe)
    	       {
    	           xe.printStackTrace();
    	       }
