...
public class ExtractK {
   ...
   //  System.err.println("y:"+y+" m:"+m+" i:"+i+"Day:"+dD.getDay()+ "wc : "+ wc );
     ...
     { 
         if(s.indexOf(",TX")>0 && s.indexOf(","+(y*100+m)+" ") > 0)
         {
             ...
             if(tx_prec != null && tx_prec.ttss/100 != r.ttss/100)
             {
  ...
  static void computeRecs(Vector v,String date_s, int contact, String name)
  {
     ...
     while(e.hasMoreElements())
     {
        ...
        end_price = r.price;
        if(r.price > high_price)
        {
           high_price = r.price;
        }
        if(r.price < low_price)
        {
           low_price = r.price;
        }
        count += r.ra;
     ...
     System.out.println(name+"|"+date_s+"|"+s_start+"|"+open_price+"|"+high_price+"|"
                        +low_price+"|"+end_price+"|"+(count/2)+"|"+seq+"|");
  }
}
